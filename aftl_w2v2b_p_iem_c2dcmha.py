...
class balanced_sampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(max(len(pIndex0), len(pIndex1), len(pIndex2), len(pIndex3))):
            iter_list.append(np.random.permutation([
                                                    pIndex0[i % len(pIndex0)],
...
def train(train_loader, valid_loader, test_loader, valid_label, test_label,
          pernums_valid, pernums_test, valid_count, test_count, fi, ri):
    ...
    for epoch in range(num_epoch):
        ...
        if valid_acc_uw > best_valid_ac:
            ...
            if not os.path.isdir(checkpoint):
                os.mkdir(checkpoint)
            torch.save({"state_dict": model_.state_dict(),
    ...
    return best_valid_ac, best_valid_pr, best_valid_wa, best_valid_re, \
...
for fold in range(10):
    ...
    train_idx = [str(fi_) for fo in range(10) if fo not in [fold, (fold+1) % 9]
                 for fi_ in ses[fo]]
    train_data, train_pros, train_label, train_label_pf, train_sample, \
        train_emt = read_IEMOCAP(audio_indexes=train_idx,
                                 emotion_steps=emotion_steps[fold],
                                 is_training=True, filter_num=768)
    test_data, test_pros, test_label, test_label_pf, test_sample, test_emt = \
        read_IEMOCAP(audio_indexes=test_idx, emotion_steps=em_steps_test[fold],
                     is_training=False, filter_num=768)
    eval_data, eval_pros, eval_label, eval_label_pf, eval_sample, eval_emt = \
        read_IEMOCAP(audio_indexes=eval_idx, emotion_steps=em_steps_test[fold],
                     is_training=False, filter_num=768)
    ...
    test_loader = DataLoader(data_emo(test_data, test_pros,
                                      test_label.to(torch.long)),
                             batch_size=batch_size, shuffle=False,
                             sampler=None, batch_sampler=None, num_workers=0,
                             pin_memory=False, drop_last=False)
    eval_loader = DataLoader(data_emo(eval_data, eval_pros,
                                      eval_label.to(torch.long)),
                             batch_size=batch_size, shuffle=False,
    ...
    for i in range(1):
        best_valid_ac, best_valid_pr, best_valid_wa, best_valid_re, \
            best_valid_fs, test_ac, test_pr, test_wa, test_rec, test_fs, \
            best_valid_conf, test_conf = train(train_loader, eval_loader,
                                               test_loader,
                                               valid_label=eval_label_pf,
                                               test_label=test_label_pf,
                                               pernums_valid=eval_sample,
                                               pernums_test=test_sample,
                                               valid_count=eval_label.shape[0],
                                               test_count=test_label.shape[0],
                                               fi=str(fold), ri=str(i).zfill(2)
                                               )
...
labels = ['ang', 'sad', 'hap', 'neu']

average_conf_matrix = np.average(total_metrics['contst'], axis=0)
row_sums = average_conf_matrix.sum(axis=1)
normalized_matrix = average_conf_matrix / row_sums[:, np.newaxis] * 100
...
metrics = [calculate_metrics(cm) for cm in total_metrics['contst']]
average_metrics = calculate_metrics(average_conf_matrix)


# Step 3: Visualization
plt.figure(figsize=(10, 8))
ax = sns.heatmap(normalized_matrix, annot=True, cmap='Blues', fmt='.2f',
                 xticklabels=labels, yticklabels=labels, annot_kws={"size": 26})
