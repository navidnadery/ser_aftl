...
src_path = input("please enther the absolout path for EMODB wav files on your system\n(e.g. '/home/EMODB/wav/' )\n")
...
if con == "y":
    ...
    for f, g in enumerate(['03', '08', '09', '10', '11', '12', '13', '14', '15', '16']):
...
def read_EMODB(audio_indexes, emotion_steps, is_training, filter_num):
    sample_num = 2000
...
emotion_steps = [{'F':100, 'W':45, 'N':timesteps, 'T':45 }]*10 # F9
em_steps_test = [{'F':timesteps, 'W':timesteps, 'N':timesteps, 'T':timesteps}]*10 # F9
...
clip = 0
verbos = 0
def train(train_loader, test_loader, test_label, pernums_test, test_count, chpath = '', fnum = ''):
    ...
    torch.save({"state_dict": model_.state_dict(), "classifier": criterion.state_dict()}, os.path.join(checkpoint, chpath[:-4]+str(fnum)+"_emodb.pth"))
...
for f_, fold in enumerate(range(0,10,2)):
    ...
    test_idx = [str(fi_) for fo in range(10) if fo not in [fold, (fold+1)] for fi_ in ses[fo]]
    train_data, train_pros, train_label, train_label_pf, train_sample, train_emt =       read_EMODB(audio_indexes = train_idx, emotion_steps = emotion_steps[fold], is_training = True, filter_num = 768)
    test_data, test_pros, test_label, test_label_pf, test_sample, test_emt =       read_EMODB(audio_indexes = test_idx, emotion_steps = em_steps_test[fold], is_training = False, filter_num = 768)
    ...
    test_ac, test_pr, test_wa, test_re, test_fs, test_conf = train(train_loader, test_loader, test_label = test_label_pf, pernums_test = test_sample, test_count = test_label.shape[0], chpath = chpoint, fnum = f_)
